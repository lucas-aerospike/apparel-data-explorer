...
class OpenAIGateway:
    DEFAULT_SYSTEM_ROLE = """You are a vintage fashion recommender for women's dresses.
    ...
    def __init__(self, system_role=DEFAULT_SYSTEM_ROLE):
        self.system_role = system_role

    def get_system_role(self):
        return self.system_role

    def recommend(self, exemplar_id, catalog):
        client = OpenAI(
            api_key="")
        """
        ...
        """
        exemplar = next((i for i in catalog if i["itemId"] == exemplar_id),
                        None)
        if not exemplar:
        ...
        user_role = {"exemplar": exemplar, "inventory": inventory}
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": json.dumps(user_role)},
            ],
            response_format={
                "type": "json_schema",
        ...
        print(
            f"OpenAPI call complete. Round trip time = {(end - start):.3f} sec.")
